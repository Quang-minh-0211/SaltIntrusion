...
# ─────────────────────────────────────────────────────────────
@st.cache_resource
def load_model_and_scalers(model_name, lookback, horizon, salonly):
    lb_h = lookback * 2
    hz_h = horizon * 2
    if salonly_mode:
        tag = (f"SC{3}_{'_'.join(STATIONS)}_to_{TARGET_STATION}"
            f"_{model_name}_lb{lb_h}h_hz{hz_h}h_salonly")
    else:
    ...
    json_path = r"C:\Users\HP\Desktop\Đồ án\src\src_code\outputs\gridsearch\best_params.json"
    ...
    # Lấy cấu hình của mô hình hiện tại
...
# PREDICTION
# ─────────────────────────────────────────────────────────────
def predict(model, scX, scY, df_input, lookback, horizon, salonly=False):
    """
    df_input: DataFrame với cột Time + 6 raw features.
    """
    sal_cols   = [f"Salinity_{s}" for s in STATIONS]
    all_feats  = sal_cols if salonly else sal_cols + FEATURE_COLS
...
with st.sidebar:
    ...
    salonly_mode = st.checkbox("Only Salinity Data", value=False)

    st.markdown("---")
    st.markdown(f"""
    <div class="input-info">
        <strong>Yêu cầu dữ liệu:</strong><br>
        Số dòng tối thiểu: <strong>{lookback_steps}</strong><br>
...
if df_input is not None:
    ...
    required_cols = ([f"Salinity_{s}" for s in STATIONS]
                 if salonly_mode
                 else RAW_INPUT_COLS)
    ...
    if missing_cols:
        ...
        model, scX, scY, err = load_model_and_scalers(
            model_choice, lookback_steps, horizon_steps, salonly_mode
        )
        if err:
            st.error(err)
        else:
            ...
            if pred_err:
                ...
                with col1:
                    st.markdown(
                        ...
                        f'<div class="metric-label" style="color:#ffffff">Mô hình</div>'
                ...
                df_result = pd.DataFrame({
                    ...
                    "Độ mặn dự báo (‰)": [float(round(float(v), 2)) for v in pred_vals],
                })
                ...
                st.dataframe(
                    ...
                    column_config={
                        "Độ mặn dự báo (‰)": st.column_config.ProgressColumn(
                            min_value=0,
                            max_value=float(max(pred_vals.max() * 1.2, 5.0)),
                            format="%.2f",
                        )
                    },
                )
